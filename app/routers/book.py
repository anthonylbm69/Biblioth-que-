...
def search_books(
    ...
    if author_name:
        statement = statement.where(or_(Author.first_name.ilike(f"%{author_name}%"), Author.last_name.ilike(f"%{author_name}%")))
    if category:
    ...
    return PaginatedResponse(items=books_with_authors, total=total, page=page, page_size=page_size, total_pages=(total + page_size - 1) // page_size)
...
def delete_book(book_id: int, session: SessionDep):
    ...
    active_loans = session.exec(select(func.count()).where(Loan.book_id == book_id, or_(Loan.status == LoanStatus.ACTIVE, Loan.status == LoanStatus.LATE))).one()
    if active_loans > 0:
        raise HTTPException(status_code=400, detail="Emprunts en cours, suppression impossible")
...
def get_book_by_isbn(
    ...
    statement = select(Book, Author).join(Author, Book.author_id == Author.id).where(Book.isbn == isbn.strip())
    result = session.exec(statement).first()

    if not result:
        raise HTTPException(status_code=404, detail=f"Livre avec l'ISBN {isbn} non trouvé")
...
def search_books_by_year(
    ...
    year_exact: Optional[int] = Query(None, description="Année précise (ignore min/max si rempli)"),
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
):
    statement = select(Book, Author).join(Author, Book.author_id == Author.id)
